...
void errorCheck(int E, char *message)
{
    if (E)
    {
       printf("%s\n",message);
...
int main(int argc, char *argv[]) 
{
    int fd;
    struct stat s;
    int32_t *file_in_memory;
    size_t size;

    clock_t start = clock();
    fd = open(argv[1], O_RDONLY, S_IRUSR);
    errorCheck(fd < 0, "File could not be opened");    //Check if opened properly
    fstat(fd, &s);
    size = s.st_size;
    errorCheck(size < img_size, "Incorrect file size");
    file_in_memory = mmap(NULL, img_size, PROT_READ, MAP_PRIVATE, fd, 0);
    errorCheck(file_in_memory == MAP_FAILED, "Mapping has failed");
